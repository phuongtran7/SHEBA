...
auto MakeRequest(const utility::string_t& uri) -> web::http::http_request {
	...
	request.headers().add(L"Authorization", conversions::to_string_t(formated_token_string));
	request.set_request_uri(uri.c_str());
	return request;
}

auto GetAllPublicRepo(web::http::client::http_client& client) -> std::vector<std::string> {
	using namespace utility;
	using namespace web;
	using namespace http;
	...
	pplx::task<std::vector<std::string>> request_task = client.request(MakeRequest(builder.to_string()))
...
auto BuildDatabase(web::http::client::http_client& client, const std::vector<std::string>& input) -> std::vector<RepoInfo> {
	using namespace utility;
	using namespace web;
	using namespace http;
...
auto main() -> int
{
	...
	auto repos = GetAllPublicRepo(client_);
	auto database = BuildDatabase(client_, repos);
	DrawTable(database);
	std::getchar();
}
